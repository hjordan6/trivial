...
export const useRunStore = defineStore('run', () => {
  ...
  function shareText(url:string){const rows=[0,1,2].map(pos=>puzzle.value!.questions.filter(q=>q.topic_position===pos).map(q=>symbol(answerMap.value.get(q.question_id)?.outcome)).join(''));const elapsed=run.value?.completed_at?Math.max(0,Math.round((Date.parse(run.value.completed_at)-Date.parse(run.value.started_at))/1000)):0;return `Trivial ${puzzle.value!.date}\n${rows.join('\n')}\n${score.value}/9 in ${Math.floor(elapsed/60)}:${String(elapsed%60).padStart(2,'0')}\n${url}`}

  return {puzzle,run,loading,error,stats,answerMap,remainingMs,complete,score,load,start,reveal,answer,finish,share,loadStats,resync,symbol,shareText}
})
