    ...
    <template v-else>
      ...
      <template v-else-if="account.step === 'code'">
        ...
        <form @submit.prevent="submitCode">
          ...
          <!-- Between the field and the button, where someone waiting on a
               code that has not arrived is already looking -- and a callout
               rather than a line, because this is the single most common reason
               a sign-in stalls: the mail arrived, the player never saw it, and
               from here that looks identical to the mail never being sent. The
               second sentence is the part that fixes it for good; marking one
               message keeps every later code out of spam.

               Temporary, along with the same wording in the email and in
               codeSentMessage. The sending domain is new, so Gmail still files
               some codes as spam while its reputation builds, and all three
               come out once delivery settles. -->
